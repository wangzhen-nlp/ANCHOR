...
class DistanceChecker:
    ...
    """
    def __init__(self, site_graph_file: str = 'site_graph.json', knn: int = 5, distance_k: float = 5000):
        """
        Args:
...
def load_site_graph(graph_file: str = 'site_graph.json') -> dict:
    """
    加载站点连接图
...
def load_ne_graph(graph_file: str = 'ne_graph.json') -> dict:
    """
...
def load_ne_info(data_dir: str = "SYS_NE_0306") -> dict:
    """
    从SYS_NE_0306加载NE信息
...
def load_rules(excel_file: str = 'CROSS_alarm_propagation.xlsx') -> list:
    """
    加载告警传播规则
    """
    import pandas as pd
...
def build_propagation_graph(alarms: list, rules: list, ne_graph: dict,
                            ...
                            no_topo: bool = False, record_rule: bool = False) -> dict:
    """
    根据告警传播规则构建NE传播图
    ...
    if knn or distance_k:
        distance_checker = DistanceChecker('site_graph.json', knn=knn or 5, distance_k=distance_k or 5000)
...
def main():
    ...
    parser.add_argument('--rules', type=str, default='CROSS_alarm_propagation.xlsx',
                        help='规则文件')
    parser.add_argument('--ne-graph', type=str, default='ne_graph.json',
                        help='NE连接图文件 (包含link信息)')
    parser.add_argument('--site-graph', type=str, default='site_graph.json',
                        help='站点连接图文件 (用于距离计算)')
    parser.add_argument('--ne-dir', type=str, default='SYS_NE_0306',
                        help='NE数据目录')
