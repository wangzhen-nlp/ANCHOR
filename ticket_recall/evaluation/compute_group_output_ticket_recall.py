...
def main():
    parser = ArgumentParser(
        description="以上限结果里的 associated_sites 作为 gold，输出故障组方法召回到的站点/告警和未召回站点/告警"
    )
    parser.add_argument(
        "group_output",
    ...
    parser.add_argument(
        ...
        help="输出 JSON 文件，默认: group_output_ticket_recall.json",
    )
    parser.add_argument(
        "--case-jsonl-output",
    ...
    parser.add_argument(
        "--use-site-list",
        action="store_true",
        help="用 group_info.site_list 作为预测站点；默认只用实际有告警的站点",
    )
    parser.add_argument(
        "--ultimate-only",
        action="store_true",
