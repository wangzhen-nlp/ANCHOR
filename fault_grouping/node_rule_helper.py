    ...
    def events_in_window(self, physical_node, reference_ts, edge_window, exclude_consumed_trigger_rule=None):
        ...
        for cached_event in self.event_getter(physical_node):
            ...
            if isinstance(cached_event, dict):
                ...
                ts, eid, alarm, alarm_source, consumed_trigger_rules, occurrence_uuid = cached_event
                segment_key = None
            ...
            if raw_event_items:
                ...
                for raw_item in raw_event_items[left_idx:right_idx]:
                    raw_event_id, raw_ts, raw_occurrence_id = raw_item
                    matched_event = {
                        "node": physical_node,
                        "ts": raw_ts,
                        "eid": raw_event_id,
            ...
            if isinstance(expected, dict):
                ...
                if collected_events:
                    ...
                    for event in collected_events:
                        ...
                        if occurrence_uuid in (None, ""):
                            raise ValueError("node event is missing required occurrence_uuid")
                        event_id = (str(event.get("eid")), str(occurrence_uuid))
