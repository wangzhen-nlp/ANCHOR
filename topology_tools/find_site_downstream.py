...
def parse_prediction_direction(edge):
    ...
    if (not upstream_site or not downstream_site) and isinstance(prediction, str):
        if prediction != "bidirectional" and "->" in prediction:
            upstream_site, downstream_site = prediction.split("->", 1)

    return normalize_site_id(upstream_site), normalize_site_id(downstream_site)
...
def looks_like_downstream_map(data):
    if not isinstance(data, dict):
        return False
    if "downstream_map" in data or "edges" in data or "meta" in data:
        return False
    return all(isinstance(value, (list, tuple, set)) for value in data.values())
