...
from ne_link_learning.core import (
    build_graph_context,
    generate_candidate_link_samples_for_scoring,
    load_json,
    load_ne_graph,
    vectorize_samples,
    write_json,
...
def _derive_output_path(model_file):
    if model_file.endswith(".json"):
        return model_file[:-5] + ".ranked_candidates.jsonl"
    return model_file + ".ranked_candidates.jsonl"


def _derive_summary_path(model_file):
    if model_file.endswith(".json"):
        return model_file[:-5] + ".ranked_candidates.summary.json"
    return model_file + ".ranked_candidates.summary.json"


def main():
    parser = ArgumentParser(description="用训练好的模型对当前 ne_graph 候选缺失边进行打分排序")
    ...
    parser.add_argument("--same-source-site-negatives", type=int, default=2)
    parser.add_argument("--same-target-site-negatives", type=int, default=2)
    parser.add_argument("--two-hop-target-negatives", type=int, default=2)
    parser.add_argument("--two-hop-source-negatives", type=int, default=2)
    parser.add_argument("--site-pair-negatives", type=int, default=2)
    parser.add_argument("--reverse-direction-negatives", type=int, default=1)
    parser.add_argument("--random-hard-negative-ratio", type=float, default=2.0)
    ...
    candidate_samples_raw = generate_candidate_link_samples_for_scoring(
        context=context,
    ...
    candidate_samples = [
        {
            ...
            "meta": {
                "u_ne_id": item["u_ne_id"],
                "v_ne_id": item["v_ne_id"],
                "u_site_id": item["u_site_id"],
                "v_site_id": item["v_site_id"],
